...
def main():
    ...
    parser.add_argument(
        "--init",
        action="store_true",
        help="Pass flag to initialize the pod rather than sync all replicas.",
    )
    parser.add_argument(
        "--namespace",
        type=str,
        default=None,
        help="Namespace for Pihole Statefulset, defaults to reading from /run/secrets/kubernetes.io/serviceaccount/namespace",
    )
    parser.add_argument(
        "--backup-mount",
        type=dir_path,
        default="/mnt/backup",
        help="Mount directory for backup PVC",
    )
    parser.add_argument(
        "--pihole-pvc-mount",
        type=dir_path,
        default="/mnt/pihole-pvc",
        help="Directory containing the mount for the PiHole PVC of the pod",
    )
    parser.add_argument(
        "--log-level",
        type=str,
        ...
        help="Sets the logging level to use.",
    )
    ...
    if args.init:
        initialize_sync(
            args.name, namespace, pod_name, args.pihole_pvc_mount, args.backup_mount
        )
    else:
...
def initialize_sync(
    ...
    if pod_name == f"{name}-0":
        ...
        if not path.join(pihole_pvc_mount, "gravity.db"):
            logging.warning(
                "Pihole has not yet been initialized, subsequent pods will be unable to start up until gravity.db exists and has been copied to the backup PVC by the synchronization job."
            )
        return
...
def scheduled_sync(
    name: str, namespace: str, pihole_pvc_mount: str, backup_mount: str
) -> None:
    """
...
def get_current_replica_count(name: str, namespace: str, api_instance) -> int:
    ...
    """
    try:
        statefulset_response = api_instance.read_namespaced_stateful_set_scale(
            name, namespace
        )
    except kubernetes.client.rest.ApiException as api_error:
...
def set_new_replica_amount(
    name: str, namespace: str, api_instance, replica_count: int
) -> None:
    """
    ...
    try:
        ...
        api_instance.patch_namespaced_stateful_set_scale(
            name, namespace, scale_changes, field_validation="Strict"
        )
        count = 0
        while (
            api_instance.read_namespaced_stateful_set_scale(
                name, namespace
            ).status.replicas
            != replica_count
        ):
...
def check_gravity_db_changed(source_pvc: str, dest_pvc: str) -> bool:
    ...
    if path.exists(dest_db):
        ...
        logging.warning(
            "%s does not exist, therefore %s is being treated as changed",
            dest_db,
            source_db,
        )
        return True
